...
def http_post(request):
    ...
    if chain_id == 'rinkeby':
        ...
        num_tickets = raindrops_contract.functions.totalTickets().call()

        # Grab info from all of the events and add to working list
        for i in range(num_tickets):
            ticket_info = raindrops_contract.functions.getTicketInfo(i+1).call()

            event_name = ticket_info[0]
            ticket_deposit = ticket_info[1]
            ticket_price = ticket_info[2]
            ticket_uri = ticket_info[3]
            ticket_owner = ticket_info[4]
            ticket_sale = ticket_info[5]
            ticket_redemption = ticket_info[6]

            event_names.add(event_name)
            ticket_list.append({"number": str(i+1), "name": event_name, "deposit_amount": ticket_deposit,
                                "price": ticket_price, "uri": ticket_uri, "owner": ticket_owner,
                                "for_sale": ticket_sale, "redemption_status": ticket_redemption})

        for event in event_names:
            response_data[event] = []

        for ticket in ticket_list:
            response_data[ticket["name"]].append(ticket)
